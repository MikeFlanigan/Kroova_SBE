...
// have!
#define SERVOMIN  100 // SMALLER MORE THROW AFT this is the 'minimum' pulse length count (out of 4096)
...
//    delay(1500);
  ...
  for (flap_percent = 0; flap_percent <= 1.0; flap_percent += 0.001) {
    ...
    delay(5);
    Serial.println(flap_cmd_pulse);

  }
  Serial.println(flap_cmd_pulse);

  delay(1500);
  for (flap_percent = 1.0; flap_percent >= 0.0; flap_percent -= 0.001) {
    // check resolution here
    flap_cmd_pulse = flap_percent * (MOTHMAX - MOTHMIN) + MOTHMIN;
    if (SERVOMIN < flap_cmd_pulse < SERVOMAX) {
      pwm.setPWM(servonum, 0, flap_cmd_pulse);
